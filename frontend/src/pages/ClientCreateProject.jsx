...
export default function ClientCreateProject() {
  ...
  const [f, setF] = useState({
    title: "", project_type: "Reel", priority: "medium",
  ...
  const submit = async () => {
    setErr(""); setBusy(true);
    try {
      const payload = { ...f, num_videos: Number(f.num_videos) || 1 };
  ...
  return (
    <Layout allowed={["client"]}>
      <PageHeader label="Client / Create" title="New Project" subtitle="Submit your brief — admin approves before assignment.">
        <button data-testid="submit-project-button" onClick={submit} disabled={busy} className="px-4 py-2 bg-white text-black rounded-md text-sm font-medium hover:bg-zinc-200 disabled:opacity-50">
      ...
      <div className="space-y-6 max-w-3xl">
        <section className="border border-white/10 rounded-md p-5 bg-zinc-900/30">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <Field label="Title"><input data-testid="cp-title" className={inp} value={f.title} onChange={e => setF({ ...f, title: e.target.value })} /></Field>
            <Field label="Type">
              <select data-testid="cp-type" className={inp} value={f.project_type} onChange={e => setF({ ...f, project_type: e.target.value })}>
                {["Reel","Ad","Podcast","Documentary","Vlog","YouTube","Short"].map(o => <option key={o}>{o}</option>)}
              </select>
            </Field>
            <Field label="Priority">
              <select data-testid="cp-priority" className={inp} value={f.priority} onChange={e => setF({ ...f, priority: e.target.value })}>
                {["low","medium","high","urgent"].map(o => <option key={o}>{o}</option>)}
              </select>
            </Field>
            <Field label="Deadline"><input data-testid="cp-deadline" type="date" className={inp} value={f.deadline} onChange={e => setF({ ...f, deadline: e.target.value })} /></Field>
          </div>
        </section>
        ...
        <section className="border border-white/10 rounded-md p-5 bg-zinc-900/30">
          ...
          <div className="grid md:grid-cols-4 gap-4">
            ...
            <Field label="Aspect"><input className={inp} value={f.aspect_ratio} onChange={e => setF({ ...f, aspect_ratio: e.target.value })} /></Field>
          </div>
        </section>
        ...
        <section className="border border-white/10 rounded-md p-5 bg-zinc-900/30">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <Field label="Footages URL"><input className={inp} value={f.footages_url} onChange={e => setF({ ...f, footages_url: e.target.value })} placeholder="Drive / Dropbox link" /></Field>
            <Field label="Script URL"><input className={inp} value={f.script_url} onChange={e => setF({ ...f, script_url: e.target.value })} placeholder="Doc link" /></Field>
          </div>
        </section>
        ...
        <section className="border border-white/10 rounded-md p-5 bg-zinc-900/30">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <Field label="Goal"><textarea data-testid="cp-goal" rows={2} className={inp} value={f.brief_goal} onChange={e => setF({ ...f, brief_goal: e.target.value })} /></Field>

...
import { createLocalFinanceRequest, getLocalApprovedFinanceAccess } from "../../lib/moderatorFinanceRequestStore";
...
const STATUS_LABELS = { available: "available", active: "active", pending: "awaiting admin approval", submitted: "awaiting admin approval", awaiting_admin_approval: "awaiting admin approval", pending_admin_approval: "awaiting admin approval", client_review: "client review", revision: "revision", completed: "completed" };
const STATUS_COLORS = { available: "#71717A", active: "#3B82F6", pending: "#F59E0B", submitted: "#F59E0B", awaiting_admin_approval: "#F59E0B", pending_admin_approval: "#F59E0B", client_review: "#8B5CF6", revision: "#EF4444", completed: "#10B981", draft: "#52525B" };
const metricThemes = { blue: ["rgba(59,130,246,.30)", "rgba(59,130,246,.10)", "text-blue-300", "bg-blue-400"], amber: ["rgba(245,158,11,.34)", "rgba(245,158,11,.11)", "text-amber-300", "bg-amber-400"], emerald: ["rgba(16,185,129,.30)", "rgba(16,185,129,.10)", "text-emerald-300", "bg-emerald-400"], red: ["rgba(239,68,68,.32)", "rgba(239,68,68,.10)", "text-red-300", "bg-red-400"], purple: ["rgba(168,85,247,.30)", "rgba(168,85,247,.10)", "text-purple-300", "bg-purple-400"], zinc: ["rgba(255,255,255,.12)", "rgba(255,255,255,.045)", "text-zinc-100", "bg-zinc-400"] };
...
function statusLabel(value) { return STATUS_LABELS[value] || value || "unknown"; }
function statusGroup(status) { if (["pending", "submitted", "admin_review", "pending_admin_approval"].includes(status)) return "awaiting_admin_approval"; return status || "available"; }

function Badge({ children, tone = "default" }) {
  const tones = { default: "border-white/10 bg-white/5 text-zinc-300", blue: "border-blue-500/25 bg-blue-500/10 text-blue-200", good: "border-emerald-500/25 bg-emerald-500/10 text-emerald-300" };
...
function buildDaily(tasks) {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    rows.push({ date, tasks: dayTasks.length, active: dayTasks.filter((t) => statusGroup(t.status || t.current_status) === "active").length, reviews: dayTasks.filter((t) => ["awaiting_admin_approval", "client_review"].includes(statusGroup(t.status || t.current_status))).length, revisions: dayTasks.filter((t) => statusGroup(t.status || t.current_status) === "revision").length });
  }
  return rows;
}

function buildFinanceFromTasks(tasks) {
  const now = new Date();
  ...
  return { monthly_revenue: revenue, monthly_profit: revenue - cost, daily: buildDaily(tasks) };
}
...
export default function ModeratorCommandOverview() {
  ...
  const load = async () => {
    const [dashRes, profileRes, financeRes, trendsRes, tasksRes] = await Promise.allSettled([api.get("/moderator/dashboard"), api.get("/moderator/profile"), api.get("/workflow/moderator-finance/access"), api.get("/stats/trends"), api.get("/tasks")]);
    if (dashRes.status === "fulfilled") setDashboard(dashRes.value.data || {});
    if (profileRes.status === "fulfilled") setProfile(profileRes.value.data || null);
    if (trendsRes.status === "fulfilled") setAdminTrends(trendsRes.value.data || null);
    if (tasksRes.status === "fulfilled") setAdminTasks(Array.isArray(tasksRes.value.data) ? tasksRes.value.data : []);
    if (financeRes.status === "fulfilled") setFinance(financeRes.value.data || { finance_access: { allowed: false }, daily: [] });
    if (financeRes.status !== "fulfilled") {
      const local = getLocalApprovedFinanceAccess(user);
      if (local) setFinance({ finance_access: { allowed: true, expires_at: local.expires_at }, monthly_revenue: local.monthly_revenue, monthly_profit: local.monthly_profit, daily: local.daily || [] });
    }
  };
  ...
  const projects = adminTasks.length ? adminTasks : dashboard.managed_projects || [];
  ...
  const statusData = useMemo(() => {
    if (adminTrends?.status_breakdown?.length) return adminTrends.status_breakdown;
    if (dashboard.status_breakdown?.length) return dashboard.status_breakdown;
    const counts = { available: 0, active: 0, awaiting_admin_approval: 0, client_review: 0, revision: 0, completed: 0 };
    projects.forEach((p) => { const key = statusGroup(p.status || p.current_status); counts[key] = (counts[key] || 0) + 1; });
    return Object.entries(counts).map(([name, value]) => ({ name, value }));
  }, [adminTrends, dashboard.status_breakdown, projects]);

  const derivedCounts = useMemo(() => {
    const map = new Map(statusData.map((item) => [statusGroup(item.name), Number(item.value || 0)]));
    return { active: map.get("active") || 0, awaiting: (map.get("awaiting_admin_approval") || 0) + (map.get("pending") || 0) + (map.get("submitted") || 0), clientReview: map.get("client_review") || 0, revisions: map.get("revision") || 0, completed: map.get("completed") || 0, total: statusData.reduce((sum, item) => sum + Number(item.value || 0), 0) };
  }, [statusData]);

  const trendData = accessAllowed ? (finance.daily || []) : (adminTrends?.daily || dashboard.operations_daily || buildDaily(projects));

  const requestFinance = async () => {
    try {
      const { data } = await api.post("/workflow/moderator-finance/request");
      setFinanceRequested(true);
      setNotice(data?.already_pending ? "Request already pending. Admin can approve or reject it from Team page." : "Request sent to Admin. Revenue and profit will unlock for 6 hours after approval.");
    } catch {
      createLocalFinanceRequest(user);
      setFinanceRequested(true);
      setNotice("Request created. Admin can approve or reject it from Team page.");
    }
  };

  const signOut = async () => { if (logout) await logout(); };

  return <div className="min-h-screen bg-zinc-950 text-white lg:flex"><aside className="hidden lg:flex fixed left-0 top-0 h-screen w-[228px] shrink-0 flex-col border-r border-white/10 bg-zinc-950"><div className="h-[86px] px-5 flex items-center border-b border-white/10"><NavLink to="/moderator/overview" className="flex items-center gap-3"><div className="w-9 h-9 rounded-md bg-black grid place-items-center overflow-hidden"><img src="/motionholic-logo.png" alt="Motionholic OS" className="w-8 h-8 object-contain" /></div><div><div className="text-sm font-semibold leading-tight">Motionholic OS</div><div className="text-[10px] text-zinc-500 font-mono tracking-[0.25em] uppercase">Creative Agency OS</div></div></NavLink></div><div className="px-5 pt-6 pb-3"><div className="text-[10px] text-zinc-600 font-mono tracking-[0.25em] uppercase">Moderator</div></div><nav className="flex-1 overflow-y-auto px-3 space-y-1">{navItems.map(([label, to, icon]) => <NavLink key={to} to={to} className={({ isActive }) => `flex items-center gap-3 rounded-md border-l-2 px-3 py-2.5 text-sm transition-all ${isActive ? "border-white bg-white/10 text-white" : "border-transparent text-zinc-400 hover:bg-white/5 hover:text-white"}`}><span className="w-4 text-center text-zinc-400">{icon}</span><span>{label}</span></NavLink>)}</nav><div className="border-t border-white/10 p-4"><NavLink to="/moderator/profile" className="flex w-full items-center gap-3 text-left"><div className="relative"><div className="grid h-9 w-9 place-items-center rounded-full bg-zinc-800 text-sm font-medium">{firstLetter(safeProfile.real_name)}</div><span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-zinc-950 bg-emerald-400" /></div><div className="min-w-0"><div className="truncate text-sm font-medium">{safeProfile.real_name}</div><div className="text-[10px] uppercase tracking-[0.2em] text-zinc-500">Moderator</div></div></NavLink><button onClick={signOut} className="mt-4 flex items-center gap-2 text-sm text-zinc-500 hover:text-white"><span>↳</span><span>Sign out</span></button></div></aside><main className="min-w-0 flex-1 lg:ml-[228px]"><div className="mx-auto max-w-[1480px] px-4 py-5 sm:px-6 lg:px-7 lg:py-7"><div className="mb-6"><div className="text-[10px] uppercase tracking-[0.25em] text-zinc-500">Moderator / Overview</div><h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">Command Center</h1><p className="mt-2 text-sm text-zinc-400">Operations health, review flow, workload, and risk.</p></div>{notice && <div className="mb-4 rounded-xl border border-blue-500/20 bg-blue-500/10 px-4 py-3 text-sm text-blue-200">{notice}</div>}<div className="mb-6 rounded-xl border border-blue-500/20 bg-gradient-to-br from-blue-500/10 via-zinc-900/30 to-zinc-950 p-5 flex items-center gap-4"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-blue-500/15 text-2xl">🛡️</div><div className="flex-1"><div className="text-[10px] uppercase tracking-[0.25em] text-blue-300">Operations Manager</div><div className="mt-1 text-2xl font-bold">{safeProfile.real_name}</div><div className="text-sm text-zinc-400">Limited access · Daily agency operations</div></div><div className="hidden text-right sm:block"><Badge tone="blue">Limited Operations Access</Badge>{accessAllowed && <div className="mt-2 text-xs text-emerald-300">Finance visible until {formatDate(finance.finance_access.expires_at)}</div>}</div></div><div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4"><MetricCard label="Total Projects" value={derivedCounts.total} color="purple" helper="Visible project records" /><MetricCard label="Active" value={derivedCounts.active} color="amber" helper="Currently moving" /><MetricCard label="Awaiting Admin Approval" value={derivedCounts.awaiting} color="blue" helper="Needs admin review" /><MetricCard label="Client Review" value={derivedCounts.clientReview} color="purple" helper="With client" /><MetricCard label="Monthly Revenue" value={formatMoney(finance.monthly_revenue)} color="blue" helper={accessAllowed ? "6-hour approved view" : "Admin approval required"} locked={!accessAllowed} onRequest={requestFinance} requested={financeRequested} /><MetricCard label="Monthly Profit" value={formatMoney(finance.monthly_profit)} color="emerald" helper={accessAllowed ? "6-hour approved view" : "Admin approval required"} locked={!accessAllowed} onRequest={requestFinance} requested={financeRequested} /><MetricCard label="Revisions" value={derivedCounts.revisions} color="red" helper="Needs changes" /><MetricCard label="Completed" value={derivedCounts.completed} color="emerald" helper="Delivered projects" /></div><div className="mb-6 grid gap-4 lg:grid-cols-3"><Panel title={accessAllowed ? "Revenue · Profit · Tasks (last 30 days)" : "Operations Trend (real-time, finance locked)"} className="lg:col-span-2 border-blue-500/15"><ResponsiveContainer width="100%" height={260}><LineChart data={trendData}><CartesianGrid strokeDasharray="3 3" stroke="#27272A" /><XAxis dataKey="date" stroke="#71717A" fontSize={10} tickFormatter={(d) => d?.slice(5)} /><YAxis stroke="#71717A" fontSize={10} /><Tooltip contentStyle={{ backgroundColor: "#18181B", border: "1px solid #27272A", borderRadius: 6 }} />{accessAllowed && <Line type="monotone" dataKey="revenue" stroke="#10B981" strokeWidth={2} dot={false} />}{accessAllowed && <Line type="monotone" dataKey="profit" stroke="#3B82F6" strokeWidth={2} dot={false} />}<Line type="monotone" dataKey="tasks" stroke="#F59E0B" strokeWidth={2} dot={false} />{!accessAllowed && <Line type="monotone" dataKey="active" stroke="#3B82F6" strokeWidth={2} dot={false} />}{!accessAllowed && <Line type="monotone" dataKey="reviews" stroke="#8B5CF6" strokeWidth={2} dot={false} />}{!accessAllowed && <Line type="monotone" dataKey="revisions" stroke="#EF4444" strokeWidth={2} dot={false} />}</LineChart></ResponsiveContainer>{!accessAllowed && <div className="mt-3"><Empty>Revenue and profit are hidden. The graph shows real-time project operations only.</Empty></div>}</Panel><Panel title="Status Breakdown" className="border-purple-500/15"><ResponsiveContainer width="100%" height={260}><PieChart><Pie data={statusData.filter((s) => Number(s.value || 0) > 0)} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, value }) => `${statusLabel(name)} (${value})`}>{statusData.filter((s) => Number(s.value || 0) > 0).map((s, i) => <Cell key={i} fill={STATUS_COLORS[statusGroup(s.name)] || STATUS_COLORS[s.name] || "#52525B"} />)}</Pie></PieChart></ResponsiveContainer>{statusData.every((s) => Number(s.value || 0) === 0) && <Empty>No project status data yet.</Empty>}</Panel></div><div className="grid gap-4 lg:grid-cols-2"><Panel title="Status Matrix">{statusData.map((s) => <div key={s.name} className="mb-2 flex items-center justify-between rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm"><span>{statusLabel(s.name)}</span><span className="font-mono text-zinc-300">{s.value}</span></div>)}</Panel><Panel title="Team Workload">{workload.length ? workload.slice(0, 5).map((w) => <div key={w.team_member_name} className="mb-2 flex items-center justify-between rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm"><span>{w.team_member_name}</span><span className="text-zinc-500">{w.active_tasks} active · {w.availability}</span></div>) : <Empty>No team workload data yet.</Empty>}</Panel><Panel title="Client Messages">{messages.length ? messages.slice(0, 5).map((m) => <div key={m.id} className="mb-2 rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm"><div>{m.sender_name || m.sender_role || "Client"}</div><div className="text-xs text-zinc-500 truncate">{m.content}</div></div>) : <Empty>No client messages waiting.</Empty>}</Panel><Panel title="Recent Activity">{activity.length ? activity.slice(0, 6).map((a) => <div key={a.id || a.created_at} className="mb-2 rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm"><div>{a.action || "Moderator activity"}</div><div className="text-xs text-zinc-600">{formatDate(a.created_at)}</div></div>) : <Empty>No recent activity yet.</Empty>}</Panel></div></div></main></div>;
}

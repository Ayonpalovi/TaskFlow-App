...
const STATUS_LABELS = { available: "available", active: "active", pending: "awaiting admin approval", submitted: "awaiting admin approval", awaiting_admin_approval: "awaiting admin approval", client_review: "client review", revision: "revision", completed: "completed" };
const STATUS_COLORS = { available: "#71717A", active: "#3B82F6", pending: "#F59E0B", submitted: "#F59E0B", awaiting_admin_approval: "#F59E0B", client_review: "#8B5CF6", revision: "#EF4444", completed: "#10B981", draft: "#52525B" };
...
function statusGroup(status) { if (["pending", "submitted", "admin_review"].includes(status)) return "awaiting_admin_approval"; return status || "available"; }
...
export default function ModeratorCommandOverview() {
  ...
  const load = async () => {
    const [dashRes, profileRes, financeRes, trendsRes, tasksRes] = await Promise.allSettled([api.get("/moderator/dashboard"), api.get("/moderator/profile"), api.get("/moderator/finance-access"), api.get("/stats/trends"), api.get("/tasks")]);

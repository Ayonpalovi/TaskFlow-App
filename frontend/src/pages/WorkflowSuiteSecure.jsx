...
function editorPrivateName(editor, isAdmin) {
  if (isAdmin) {
    const real = editor.real_name || editor.display_name || editor.name || "Unnamed editor";
    return editor.anime_name ? `${real} (${editor.anime_name})` : real;
  }
...
export default function WorkflowSuiteSecure() {
  ...
  const tabs = [
    ...
    ["onboarding", "Brand Profile", ["admin", "client"]],
    ...
    <div className="grid lg:grid-cols-[280px,1fr] gap-5">
      ...
      <section className="space-y-5">
        {activeTab === "onboarding" && !isEditor && <div className="grid xl:grid-cols-[1fr,460px] gap-5">
          {(isAdmin || isClient) && <Panel title="Client Onboarding Form" subtitle="Admin can manage all client brand profiles. Clients can update their own.">
          ...
          <Panel title="Client Brand Profiles" subtitle={isAdmin ? "Admin-only detailed view of all clients." : "Your saved brand profile."}>
            {visibleProfiles.length === 0 ? <Empty title="No brand profile yet" /> : visibleProfiles.map((p) => <div key={p.id} className="border border-white/10 rounded-lg p-4 mb-3 bg-black/20">
              <div className="flex justify-between gap-3"><div><div className="font-semibold">{p.brand_name || "Untitled Brand"}</div><div className="text-xs text-zinc-500 mt-1">Client ID: {p.client_id || "—"}</div></div>{(isAdmin || isClient) && <button onClick={() => setBrandForm({ ...brandForm, ...p })} className="text-xs text-blue-400">Edit</button>}</div>

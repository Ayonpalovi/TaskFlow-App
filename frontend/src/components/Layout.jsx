        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!loading && items.length === 0 && (
            <div className="p-8 text-center">
              ...
              <div className="text-xs text-zinc-600 mt-1">You are all caught up.</div>
            </div>
          )}
          ...
          {!loading && items.map((notification) => {
            const content = (
              <div
                ...
                className={`px-5 py-4 border-b border-white/5 hover:bg-white/5 cursor-pointer ${notification.read ? "opacity-60" : "opacity-100"}`}
              >
  ...
  const loadUnreadCount = useCallback(async () => {
    ...
    try {
      ...
      data.forEach((notification) => {
        ...
        if (hasExisting && !nextKnown.has(notification.id) && !notification.read && pushEnabled && permission === "granted") {
    ...
    try {
      ...
      if (result === "granted") {
        ...
        showBrowserNotification({
          id: "motionholic-os-alerts-enabled",
          title: "Motionholic OS alerts enabled",
          body: "Desktop alerts are now active for this browser.",
        });
      } else {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white lg:flex">
        ...
        <div className="h-[86px] px-5 flex items-center border-b border-white/10">
          <Link to={`/${user.role}`} className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] text-zinc-500 font-mono tracking-[0.25em] uppercase">Creative Agency OS</div>
            </div>
          </Link>
        </div>

        <div className="px-5 pt-6 pb-3">
          <div className="text-[10px] text-zinc-600 font-mono tracking-[0.25em] uppercase">{roleLabel(user.role)}</div>
        </div>
      ...
      <header className="lg:hidden sticky top-0 z-40 h-16 bg-zinc-950/95 backdrop-blur border-b border-white/10 px-4 flex items-center justify-between">
        ...
        <button type="button" onClick={() => setNotificationsOpen(true)} className="relative w-10 h-10 rounded-md border border-white/10 grid place-items-center text-zinc-300">
          🔔
        ...
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-[285px] max-w-[85vw] bg-zinc-950 border-r border-white/10 flex flex-col">
            <div className="h-16 px-4 flex items-center justify-between border-b border-white/10">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-zinc-500 uppercase tracking-[0.2em]">{roleLabel(user.role)}</div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto py-4">
              <NavItems nav={nav} userRole={user.role} onNavigate={() => setMobileMenuOpen(false)} />
            </div>
      ...
      <main className="lg:ml-[228px] flex-1 min-w-0">
        <div className="px-4 py-5 sm:px-6 lg:px-7 lg:py-7">{children}</div>
      </main>
...
export function Badge({ children, tone = "default" }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-md border text-xs ${tones[tone] || tones.default}`}>
      {children}
    </span>
  );
}
...
export function MetricCard({ label, value, tone = "default", subtitle }) {
  ...
  return (
    <div className="border border-white/10 rounded-md bg-zinc-900/30 p-4 lg:p-5">
      ...
      <div className={`font-mono text-2xl lg:text-3xl font-semibold ${valueTone[tone] || valueTone.default}`}>{value}</div>

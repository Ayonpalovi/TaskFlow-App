...
def build_moderator_account_router(server):
    ...
    async def invite_moderator(data: ModeratorInviteIn, admin: dict = Depends(server.require_role("admin"))):
        ...
        await server.create_notification(admin["id"], "moderator_invited", f"Moderator invite created for {email}", body="Copy the setup link and send it manually.", link="/admin/absence")
        result = server.scrub_user(doc, viewer_role="admin")
